...
class ImageAgent:
    ...
    def ask(self, text, confirm, history=None, memories=None, allowed_tools=None,
            model=None, reasoning_effort=None, images=None):
...
def test_image_without_text_and_rate_limit(tmp_path):
    ...
    agent = ImageAgent()
    server = AssistantServer(("127.0.0.1", 0), agent,
                             SimpleNamespace(groq_api_key="test", groq_model="test"), memory)
    ...
    try:
        with httpx.Client(base_url=f"http://127.0.0.1:{server.server_address[1]}",
            ...
            client.post("/api/change-password", json={
                "current_password": credentials["will"], "new_password": "test-password-safe"
            })
            payload = {"message": "", "mode": "temporary", "image_consent": True,
            ...
            assert client.post("/api/chat", json={**payload, "image_consent": False}).status_code == 400
            agent.limited = True
