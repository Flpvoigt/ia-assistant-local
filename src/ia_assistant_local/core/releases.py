...
LOCK = threading.Lock()
PLANS: dict[str, dict] = {}
ALLOWED_ROOT = {"README.md", "SECURITY.md", "requirements.txt", "pyproject.toml",
                "start.ps1", "install.ps1", "start.sh", "install.sh", ".gitignore", ".env.example"}
SUFFIXES = {".py", ".html", ".js", ".css", ".json", ".md", ".svg", ".webmanifest", ".txt"}
PRIVATE = re.compile(r"(^|/)(data|\.git|\.venv|node_modules|__pycache__|\.ssh)(/|$)|\.(db|key|pem|pfx|sqlite)$", re.IGNORECASE)
SECRET = re.compile(r"gsk_[A-Za-z0-9]{20,}|sk-[A-Za-z0-9]{24,}|-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----")
...
def _git(root: Path, *args: str) -> str:
    try:
        ...
        raise RuntimeError("O Git não concluiu a operação. Verifique acesso ao remoto, "
                           "identidade de commit e estado do repositório no terminal.") from exc
...
def _snapshot(root: Path) -> dict:
    ...
    if not remote_head or remote_head[0] != head:
        raise ValueError("A main local e a remota precisam estar sincronizadas antes de lançar. Revise os commits pendentes no Git.")
    ...
    for name in tracked:
        if PRIVATE.search(name) or (Path(name).name.startswith(".env") and name != ".env.example"):
            raise ValueError("Há arquivos privados rastreados. Revise o repositório antes de publicar.")
    ...
    for name in sorted(paths):
        ...
        path = root / name
        if PRIVATE.search(name) or path.is_symlink() or not path.resolve().is_relative_to(root.resolve()):
    ...
    return {"head": head, "branch": branch, "remote": remote,
            "files": sorted(paths), "digest": digest.hexdigest()}
...
def prepare_release(root: Path, user_id: int, notes: list[str]) -> dict:
    ...
    with LOCK:
        plan = _snapshot(root)
        plan.update(version=next_version(release_info(root)["version"]),
                    notes=clean, user_id=user_id, created=time.monotonic(), root=str(root.resolve()))
        token = secrets.token_urlsafe(24)
        PLANS.clear()
        PLANS[token] = plan
        return {key: plan[key] for key in ("version", "notes", "files", "branch", "remote")} | {"token": token}


def publish_release(root: Path, user_id: int, token: str) -> dict:
    with LOCK:
        plan = PLANS.pop(token, None)
        if not plan or plan["user_id"] != user_id or plan["root"] != str(root.resolve()) or time.monotonic()-plan["created"] > 300:
        ...
        manifest.write_text(json.dumps({"version": plan["version"], "published": True,
        ...
        try:
            ...
            raise RuntimeError("Não foi possível criar o commit. Revise os arquivos preparados no Git; nada foi enviado.") from exc
        commit = _git(root, "rev-parse", "HEAD")
        try:
            _git(root, "push", "origin", "HEAD:refs/heads/main")
        except (RuntimeError, subprocess.TimeoutExpired):
            return {"published": False, "version": plan["version"], "commit": commit,
        ...
        return {"published": True, "version": plan["version"], "commit": commit,
